...
public class Ex1_14_2answer {
	...
	*/
	static int discount( int totalPrice ){            //アンダーバーを適切な内容に書き換えてください
		final int DISCOUNT_BORDER1 = 1000; //割引が適用される金額
    final double DISCOUNT_RATE1 = 0.1;         //割引率
		final int DISCOUNT_BORDER2 = 5000; //割引が適用される金額
    final double DISCOUNT_RATE2 = 0.2;         //割引率
		final int MAX_DISCOUNT = 5000;
		
		int priceDownTemp = 0;
		if(DISCOUNT_BORDER1 <= totalPrice && totalPrice <= DISCOUNT_BORDER2 ){
			priceDownTemp = (int)((totalPrice - DISCOUNT_BORDER1) * DISCOUNT_RATE1); //値引き金額が20％
			
		} else if(totalPrice > DISCOUNT_BORDER2){
			priceDownTemp = (int)( (DISCOUNT_BORDER2 - DISCOUNT_BORDER1) * DISCOUNT_RATE1
															+ (totalPrice - DISCOUNT_BORDER2) * DISCOUNT_RATE2); //値引き金額が10％

		}
		int priceDown = 0; //最終的な値引き額
		if(MAX_DISCOUNT < priceDownTemp ){
			priceDown = MAX_DISCOUNT;
		} else {
			priceDown = priceDownTemp;
		}
		return totalPrice - priceDown;
	}
}

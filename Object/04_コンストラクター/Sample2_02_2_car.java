class Sample2_02_2_car {
	...
	Sample2_02_2_car(String cm, String on, String cl, int sp, boolean rt) {
		...
		// コンストラクターを呼び出すときは内の処理の中で最初に行わにといけないので、２行目以降にthis()と記述するとエラーになる
		System.out.println("　　▼コンストラクタ②（引数あり）-------------------------");
		this.carModel = cm; // 車種名
		// this.ownerを使わずにowner= ownerとしてしまうと左のownerはローカル変数の方になってしまうので、
		// ローカル変数にローカル変数を入れているだけになるので、処理が終了すると全く意味がなくなってしまう。
		// だからthis.ownerでフィールド変数を呼び出してきてローカル変数の値を代入する必要がある
		// thisを付け忘れるという事故も起こりうるから、ローカル変数とフィールド変数はかぶせないように心がけよう
		// フィールド変数とローカル変数がこのようにかぶることを避けるときは、ローカル変数側は省略した名前にしても理解できるので、
		// この場合のローカル変数はcarModelを単にcmに省略してもOk
		this.owner = on; // オーナー
		this.color = cl; // 塗装色
		this.speed = sp; // 現在の速度
